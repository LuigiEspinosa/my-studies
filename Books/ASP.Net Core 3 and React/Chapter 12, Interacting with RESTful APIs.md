---
source: books
slug: books/asp-net-core-3-and-react/chapter-12-interacting-with-restful-apis
status: done
started: 2025-02-27
finished: 2025-02-27
tags: [frontend, react, rest, api-design]
code: []
kind: note
---
...
> _It is important to note that requests that contain HTTP error codes are not handled by the catch method. HTTP errors must be handled in the first then method by looking at the ok or status properties on the response parameter._

## Quiz

> [!TIP]- What is wrong with the following HTTP POST request using the `fetch` function?
>
> ```ts
...
> [!TIP]- What is wrong with the following request using the `fetch` function?
>
> ```ts
...
> [!TIP]- We have an endpoint for deleting people that only administrators have access to use. We have the users' access token in a variable called `jwt`. What is wrong with the following request?
>
> ```ts
> fetch('http://localhost:17525/api/person/1', {
>  method: 'delete',
...
> [!TIP]- In this chapter, we implemented an `AuthorizedPage` component that we could wrap around a page component so that it is only rendered for authenticated users. We could implement a similar component to wrap around components within a page so that they are only rendered for authenticated users. Have a go at implementing this.
>
> ```tsx
