...
Veeva publishes a JavaScript library to assist customers and their creative agencies with developing personalized and dynamic HTML5 content that interacts with the Veeva CRM database.

This library is supported for CLM, Engage for Portals and Engage Meeting content creation.

The following examples illustrate the potencial use cases:

- Enables content creators to access the Veeva CRM database to get information including, but not limited to the doctors name and preferences allowing the content and content journeys to be personalised (`getDataforCurrentObject`).
- Enables content creators to implement custom navigation and control the flow of the presentation and access to hidden presentations (`gotoSlide`).
- Enables the integration of Surveys directly into the HTML of a CLM Presentation, allowing for a more controlled and user-friendly interaction.
- Enables the launching of Approved Email directly from a CLM presentation to a specific email template, which allows the Rep to quickly send a relevant email before returning to CLM conversation (`launchApprovedEmail`).
...
To reach a slide within a Hidden Presentation, Content Creators must use the `gotoSlide` JavaScript function to link directly to a slide in the Hidden Presentation.
...
There are a number of standard features in a Veeva CLM Presentation that aid navigation but can also be deactivated including:
...
The purpose of this method is to allow navigation between slides. As background, each slide is its own independent entity that is not aware of another slide. As such, there is no way to use standard HTML anchors to navigate to another slide.
...
Firstly, we will explain the purpose of the content.

The first part of the content is the intro slides, which is an opportunity for the Rep to introduce the subject and pose a question. The purpose of the question is to determine the current circumstances with the HCP. The answer to the simple Yes/No question will determine which content will be shown next.
...
In responding to the Yes/No question, we can use JavaScript to determine the answer selected and then provide the correct parameters to the `gotoSlide` function. Additionally, you can use the function `gotoSlide` without the Presentation parameter to navigate to a slide within the same presentation.
